...
void ImgPw::InitProbeOptimize(ProbeSocket::ProbePara* ptrPara, ExamItem::ParaItem* ptrParaItem) {
    ...
    if (m_freqIndex == 0)
        ...
        ret = OK;
    DopplerFreq(m_freq[m_freqIndex], MIN);
...
void ImgPw::WhenPRFChanged(int index) {
    ...
    // only change wall filter's display
    //m_ptrCalc->CalcWallFilter((unsigned int*)WALL_FILTER[m_wallFilterIndex], 7);
 ...
 */
void ImgPw::SwitchHPRF(bool on, float svPos) {
    int num = 0;
    int dotBegin;
    int dotEnd;
    ...
    if(on) {
        ...
        //test
        //m_PRFIndex = m_ptrCalc->GetHPRFIndex(svBegin, svEnd);
        num = m_ptrCalc->SVPosPRFToHPRF(svBegin, svEnd, GetPRFValue(m_PRFIndex));

#if 0
        int emitDepth = m_depthToSample;
        int detalt = 1000000/GetPRFValue(m_PRFIndex) - 1000000/GetPRFValue(m_maxPRFIndex);
        printf("detalt:%d\n", detalt);
        int count = 1;
        while(detalt < 20) {
            if(count%2)
                ...
                m_PRFIndex -= 2;
            detalt = 1000000/GetPRFValue(m_PRFIndex) - 1000000/GetPRFValue(prfIndex);
            printf("count:%d, detalt:%d, PRFValue:%d\n", count, detalt, GetPRFValue(m_PRFIndex));
            count++;
        }
        emitDepth = svPos = detalt * SOUND_SPEED / 2;
        svBegin = svPos - (float)m_svLen/20;
        svEnd = svBegin + (float)m_svLen/10;
#endif
...
int ImgPw::CalcTraceFs(int wallFilterIndex, int prfIndex, int baseline, int angleCorrect) {
    double speed = SOUND_SPEED * 1000;
    int wf = WALL_FILTER_FREQ[m_PRFIndex][wallFilterIndex];//unit:hz
    int freq = GetColorFreq();
...
//解决当scale调节到使得PW_PRF[index]等于0，死机问题。
    ...
    //return PW_PRF[prfIndex];
    else
        temp = CW_PRF[prfIndex];
