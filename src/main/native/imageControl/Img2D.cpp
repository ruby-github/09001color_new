...
void Img2D::InitProbe2D(ProbeSocket::ProbePara* ptrPara, ExamItem::ParaItem* ptrParaItem) {
    ...
    m_ptrDscPara->dcaProbeR = ptrPara->r;
...
#if (defined(EMP_460) || defined(EMP_355) || EMP_322)
    if(!m_harmonic) {
...
void Img2D::UpdateSeperateScaleWhenUnfreeze(void) {
    ...
    ASSERT(lastRealB <= 4);
    if (curB != lastRealB) {
        m_scale2D[curB] = m_scale2D[lastRealB];

        if (m_zoomMeasureCtrl) {
            m_scale2DZoom[curB] = m_scale2DZoom[lastRealB];
        }
    }
}
///> /////////////////////////////////[private func]////////////////////////////
...
void Img2D::SetScale2D(double scale) {
    ...
    if (m_zoomMeasureCtrl) {
        m_scale2DZoom[curB] = m_scale2D[curB] / m_zoomScale;
    }
#if 0
