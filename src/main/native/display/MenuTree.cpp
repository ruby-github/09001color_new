...
void TreeMenu::SelectNext(GtkWidget *tree_view, EMeasureSequenceType sequence) {
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter) == TRUE) {
        if (gtk_tree_model_iter_parent(model, &iterParent, &iter) == TRUE) {
            bool existNext = SelectNext(tree_view, model, selection, &iter);
            if (sequence == MEA_SEQ_NEXT && !existNext) {
                gtk_tree_view_collapse_all(GTK_TREE_VIEW(tree_view));
                SelectNext(tree_view, model, selection, &iterParent);
            }
        } else {
            if (sequence == MEA_SEQ_NEXT)
                SelectNext(tree_view, model, selection, &iter);
        }
    }
}

bool TreeMenu::SelectNext(GtkWidget *tree_view, GtkTreeModel *model, GtkTreeSelection *selection, GtkTreeIter *iter) {
