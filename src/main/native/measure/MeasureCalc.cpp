...
double MeasureCalc::CalcVol(POINT long_axis_start, POINT long_axis_end, POINT short_axis_start, double dist) {
    ...
    double vol = (double)4/(double)3 *PI * long_axis * short_axis * z_axis / 1000.0;
...
double MeasureCalc::DCalcTraceAutoOther(char buf[], vector<POINT> vec, POINT ps, POINT ed) {
    ...
    if (TraceCalcItem.edSet) {
        sprintf(buf_tmp, "\n%s= %3.2fcm/s", _("ED"), edv);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.riSet) {
        sprintf(buf_tmp, "\n%s= %3.2f", _("RI"), ri);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.sdSet) {
        sprintf(buf_tmp, "\n%s= %3.2f", _("S/D"), sd);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.tamaxSet) {
        sprintf(buf_tmp, "\n%s= %3.2fcm/s", _("TAMAX"), tamax/10.0);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.piSet) {
        sprintf(buf_tmp, "\n%s= %3.2f", _("PI"), pi);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.timeSet) {
        sprintf(buf_tmp, "\n%s= %3.2fs", _("Time"), time);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.hrSet) {
        sprintf(buf_tmp, "\n%s= %3.2fbpm", _("HR"), hr);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.pgmaxSet) {
        sprintf(buf_tmp, "\n%s= %3.2fmmHg",_("PGmax"), pgmax);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.pgmeanSet) {
        sprintf(buf_tmp, "\n%s= %3.2fmmHg", _("PGmean"), pgmean);
        strcat(buf, buf_tmp);
    }
#else
    if (TraceCalcItem.psSet) {
        sprintf(buf_tmp, "PS= %3.2fcm/s", psv);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.edSet) {
        sprintf(buf_tmp, "\nED= %3.2fcm/s", edv);
    ...
    if (TraceCalcItem.pgmaxSet) {
        sprintf(buf_tmp, "\nPGmax= %3.2fmmHg", pgmax);
        strcat(buf, buf_tmp);
    }
    if (TraceCalcItem.pgmeanSet) {
        sprintf(buf_tmp, "\nPGmean= %3.2fmmHg", pgmean);
        strcat(buf, buf_tmp);
    }
#endif
...
bool MeasureCalc::GetPsEdInVec(vector<POINT> vec, POINT &psP, POINT &edP, int baseLine, int auto_manual, bool auto_calc) {
    ...
    int vecSizeEveryCycle =(int)((float)vecSize/cycle + 0.5);
