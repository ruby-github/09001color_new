...
void UpdateMeasure::CatHighCalc(const CalcResultInfo *calcInfo, int *calcOrder, float dataMea[], char buf[], int meaLen) {
    char buf_tmp[30];
...
void UpdateMeasure::GenDisplaySingle(const SingleItemInfo *info, float data[], ResultAttr& attr, bool inMeasure, int unit_coeffi) {
    char buf[600];
    char buf_tmp[30];
    int i, k;
...
void UpdateMeasure::GenDisplayMulti(const MultiItemInfo *info, float dataMea[], ResultAttr& attr, bool inMeasure, int unit_coeffi[]) {
    char buf[256];
    char buf_tmp[30]= {'\0'};
    int i, k;
...
void UpdateMeasure::IMTResult(SIMTPARA imt_para, ResultAttr& attr, bool update) {
    ...
    snprintf(buf, 200, "%s= %3.2fmm\n%s= %3.2fmm\n%s= %3.2fmm\n%s= %3.2fmm\n%s= %3.2f \n%s= %3.2fmm",
             _("IMT MAX "), imt_para.imt_max*scale[0], _("IMT MIN "), imt_para.imt_min*scale[0], _("IMT STD "), imt_para.imt_std*scale[0], _("IMT MEAN "), imt_para.imt_mean*scale[0],
             _("IMT QUALITY "), imt_para.imt_quality,  _("IMT LENGTH "), imt_para.imt_length*scale[0]);
