...
void D2MeasureLengthTrack::PressLeft(POINT p) {
    ...
    case 1:
        ...
        m_ptrMan->SingleMeaDataMan((float)m_length, m_itemInfo, allData, SAVE);
        m_update.GenDisplaySingle(m_itemInfo, allData, attr, false, 1);
...
void D2MeasureAreaTrack::MouseMove(POINT p) {
    ...
    case 1:
        ...
        if (m_mouse_count >= MOUSE_INTERVAL) {
            ...
            float resultTmp;
            resultTmp = (m_itemInfo->meaType == PERI_TRACK) ? m_perimeter : (float)INVALID_VAL;
...
void D2MeasureRatioDistDot::PressLeft(POINT p) {
    ...
    case 3:
        ...
        if(m_method==0) { //距离狭窄比
            ...
            m_ptrMan->SingleMeaDataMan((float)m_angusty, m_itemInfo, allData, SAVE);
            m_update.GenDisplaySingle(m_itemInfo, allData, attr, false, 1);
        } else {
            // m_update.D2RatioDist(m_dist1, m_dist2, m_ratio, attr, false);
            m_ptrMan->SingleMeaDataMan((float)m_ratio, m_itemInfo, allData, SAVE);
            m_update.GenDisplaySingle(m_itemInfo, allData, attr, false, 1);
        }
