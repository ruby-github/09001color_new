...
void ViewReport::SetIDToCalcArr(int stamp, int row, bool fixed, int id, int section, const char *title, const char *value, const char *gw, const char *edcb) {
    CalcNode_t *p = CalcArrListHead;
    while(NULL != p) {
        if(id == p->id && !strcmp(title, p->title)) {
            p->row = row;
            p->stamp = stamp;
            p->check = fixed;
...
GtkWidget* ViewReport::CreateObTable(int start, int end, int map_table[][CALC_MAX], int map_start, int fetal) {
    ...
    int ret;
    int i, j, k, m, n;
    float value[MEA_MULTI], allvalue[MEA_MULTI][MEA_TIMES];
    const SingleItemInfo *sInfo;
    const MultiItemInfo *mInfo;
    char GW[10], EDCB[10], strTmp[100];
    int scale = 1;
    gint rows = 0;
    GtkListStore *store;
...
//for user
...
#endif
            {
                //printf("aaa%d\n", CheckItemType(map_table[i-map_start][j]));
        ...
        for(j=0; j<CALC_MAX; j++) {
            ...
            if(CheckItemType(map_table[i-map_start][j])==0) {
...
void ViewReport::AddTable(GtkWidget *fixed, guint *y, const char *title, int section, int start, int end, int map_table[][CALC_MAX], int fetal) {
    ...
    switch(section) {
    case ABDO_M:
        map_start = ABD_MEA_START;
        break;
    case ADULT_M:
        map_start = ADULT_MEA_START;
        break;
    case UR_M:
        map_start = UR_MEA_START;
        break;
    case OB_M:
        map_start = OB_MEA_START;
        break;
    case GYN_M:
        map_start = GYN_MEA_START;
        break;
    case SP_M:
        map_start = SP_MEA_START;
        break;
#ifndef EMP_322
    case VS_M:
        map_start = VS_MEA_START;
        break;
#endif
    case FETAL_M:
        map_start = FETAL_MEA_START;
        break;
#ifndef EMP_322
    case TCD_M:
        map_start = TCD_MEA_START;
        break;
#endif
    case ORTHO_M:
        map_start = ORTHO_MEA_START;
        break;
#ifdef VET
    case TD_M:
        map_start = TD_MEA_START;
        break;
    case ANOB_M:
        map_start = ANOB_MEA_START;
        break;
#endif
    }
...
void ViewReport::UpdateGrowthCurve(int type) {
    MeaResult *ptrRes = MeaResult::GetInstance();
    float calc_val, value[MEA_MULTI], allvalue[MEA_MULTI][MEA_TIMES];
    int item;
...
//	printf("GrowthCurve:  LMP=%d/%d/%d, gw=%d\n", info.ob.LMPDate.year, info.ob.LMPDate.month, info.ob.LMPDate.day, gw);
    ...
    switch(flag) {
    case 0:
        DrawGrowthCurve (gs_rempen1, 32, m_dataGC, gw, 0);
        break;
    case 1:
        DrawGrowthCurve (crl_hadlock1, 43, m_dataGC,gw, 0);
        break;
    case 2:
        DrawGrowthCurve (crl_robinson1, 43, m_dataGC, gw, 0);
        break;
    case 3:
        DrawGrowthCurve (crl_hansmenn1, 43, m_dataGC, gw, 0);
        break;
    case 4:
        DrawGrowthCurve (bpd_hadlock1, 84, m_dataGC, gw, 0);
        break;
    case 5:
        DrawGrowthCurve (bpd_lasser1, 84, m_dataGC, gw, 0);
        break;
    case 6:
        DrawGrowthCurve (bpd_merz1, 84, m_dataGC, gw, 0);
        break;
    case 7:
        DrawGrowthCurve (bpd_rempen1, 84, m_dataGC, gw, 0);
        break;
    case 8:
        DrawGrowthCurve (hc_merz1, 84, m_dataGC, gw, 0);
        break;
    case 9:
        DrawGrowthCurve (hc_merz1 ,84, m_dataGC, gw, 0);
        break;
    case 10:
        DrawGrowthCurve (ac_hadlock1, 84, m_dataGC, gw, 0);
        break;
    case 11:
        DrawGrowthCurve (ac_hadlock1, 84, m_dataGC, gw, 0);
        break;
    case 12:
        DrawGrowthCurve (fl_hadlock1, 84, m_dataGC, gw, 0);
        break;
    case 13:
        DrawGrowthCurve (fl_merz1, 84, m_dataGC, gw, 0);
        break;
    case 14:
        DrawGrowthCurve (fl_jeanty1, 84, m_dataGC, gw, 0);
        break;
    case 15:
        DrawGrowthCurve (hl_jeanty1, 91, m_dataGC, gw, 0);
        break;
    case 16:
        DrawGrowthCurve (EFW_hadlock1, 70, m_dataGC, gw, 1);
        break;
    case 17:
        DrawGrowthCurve (EFW_hansmenn1, 70, m_dataGC, gw, 1);
        break;
    default:
        break;
    }
