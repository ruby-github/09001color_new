...
public:
    ...
    const char* GetIndication(void) {
        return m_strReport[0].c_str();
    }
    const char* GetComments(void) {
        return m_strReport[1].c_str();
    }
...
//ик hlx
    ...
    static gboolean HandleGrowthCurveConfigure(GtkWidget *widget, GdkEventConfigure *event, ViewReport *data) {
        data->GrowthCurveConfigure(widget, event);
        return FALSE;
    }
    static gboolean HandleGrowthCurveExpose(GtkWidget *widget, GdkEventExpose *event, ViewReport *data) {
        data->GrowthCurveExpose(widget, event);
        return FALSE;
    }
    static void HandleGrowthCurveChanged(GtkComboBox *combobox, ViewReport *data) {
        data->GrowthCurveChanged(combobox);
    }
    static void HandleSpinValueChanged(GtkSpinButton *spin, ViewReport *data) {
        data->SpinValueChanged(spin);
    }
