...
void NoteArea::KeyEvent(unsigned char keyValue) {
    ...
    if(FakeMouseButton(keyValue))
        return;
    if(FakeNumKey(keyValue))//Êý×Ö¼ü
        return;
    if(FakeAlphabet(keyValue))//×ÖÄ¸¼ü
        return;
    if(FakePunctuation(keyValue)) //·ûºÅ¼ü
        return;
            ...
            /* if(!FakeAlphabet(key_note))
              {
                  ...
                  {
                      FakePunctuation(key_note);//·ûºÅ¼ü
                  }
              }*/
#else
...
void NoteArea::EndEdit() {
    ...
    if(strlen(str) > 0) {
        ...
        //	AddTextItem(str, x-1, y+3);	//Ïò×óÏÂÎ¢µ÷Î»ÖÃ
    }
