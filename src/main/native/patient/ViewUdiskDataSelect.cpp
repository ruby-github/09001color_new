...
ViewUdiskDataSelect::ViewUdiskDataSelect() {
    m_listBranch = NULL;
    m_vecPath.clear();
    DeleteSlideFile();
}
...
ViewUdiskDataSelect* ViewUdiskDataSelect::GetInstance() {
    if (m_ptrInstance == NULL)
        m_ptrInstance = new ViewUdiskDataSelect;

    return m_ptrInstance;
}

void ViewUdiskDataSelect::CreateWindow(GtkWindow *parent) {
    GtkWidget *fixed;
    ...
    m_chkbtnDel = gtk_check_button_new_with_mnemonic(_("Delete the lastet loaded file."));
    gtk_fixed_put (GTK_FIXED (fixed), m_chkbtnDel, 20, 440);
    gtk_widget_set_size_request (m_chkbtnDel, -1, 30);

#if 0
    GtkWidget *imageSelectAll = gtk_image_new_from_stock (GTK_STOCK_APPLY, GTK_ICON_SIZE_BUTTON);
    GtkWidget *labelSelectAll = gtk_label_new_with_mnemonic (_("Select All"));
    GtkWidget *btnSelectAll = create_button_icon(labelSelectAll, imageSelectAll);
    gtk_fixed_put (GTK_FIXED (fixed), btnSelectAll, 80, 410);
    g_signal_connect ( G_OBJECT(btnSelectAll), "clicked", G_CALLBACK (on_button_select_all_clicked), this);

    GtkWidget *imageDeselect = gtk_image_new_from_stock (GTK_STOCK_CLOSE, GTK_ICON_SIZE_BUTTON);
    GtkWidget *labelDeselect = gtk_label_new_with_mnemonic (_("Deselect"));
    GtkWidget *btnDeselect = create_button_icon(labelDeselect, imageDeselect);
    gtk_fixed_put (GTK_FIXED (fixed), btnDeselect, 220, 410);
    g_signal_connect ( G_OBJECT(btnSelectAll), "clicked", G_CALLBACK (on_button_deselect_clicked), this);
#endif
...
void ViewUdiskDataSelect::DeleteSlideFile(void) {
    FileMan fm;
    fm.DelDirectory(SLIDE_PATH);
}

void ViewUdiskDataSelect::DestroyWindow(void) {
    if(g_list_length(m_listBranch) > 0) {
...
gboolean ViewUdiskDataSelect::WindowDeleteEvent(GtkWidget *widget, GdkEvent *event) {
    DestroyWindow();
    return FALSE;
}

void ViewUdiskDataSelect::KeyEvent(unsigned char keyValue) {
...
static gboolean ShowViewArchiveImgMan(gpointer data) {
    vector<string> vec;
    vec.clear();
    ViewArchiveImgMan::GetInstance()->CreateWindow(vec);
    return FALSE;
}
...
static gboolean LoadSelectedData(gpointer data) {
    ...
    while(ite < vec.end() && !cond) {
        ...
        g_object_unref(fAbs);
        gchar *strDestParent = g_build_path(G_DIR_SEPARATOR_S, SLIDE_PATH, g_file_get_basename(fParent), NULL);
...
static int CancelLoadSlide(gpointer data) {
    PRINTF("Cancel copy!\n");
    g_cancellable_cancel(cancellable);
    return 0;
}

void ViewUdiskDataSelect::BtnOKClicked(GtkButton *button) {
    //Delete the existed file first
...
void ViewUdiskDataSelect::BtnCancelClicked(GtkButton *button) {
    DestroyWindow();
}

//type: 0=root, 1=branch
GtkWidget* ViewUdiskDataSelect::create_treeview(gint type) {
    GtkWidget *treeview;
...
GtkTreeModel* ViewUdiskDataSelect::create_root_model() {
    GtkListStore *store;
...
void ViewUdiskDataSelect::ToggleData(GtkCellRendererToggle *cell, gchar *path_str) {
    GtkWidget *treeview;
...
void ViewUdiskDataSelect::UpdateRootModel(void) {
    DIR *dir;
    struct dirent *ent;
...
void ViewUdiskDataSelect::UpdateBranchModel(gint rows) {
    GList *list = g_list_first(m_listBranch);
    gtk_tree_view_set_model (GTK_TREE_VIEW(m_treeBranch), GTK_TREE_MODEL(g_list_nth_data(list, rows)));
}

GtkTreeModel* ViewUdiskDataSelect::LoadBranchModel(gchar *branch) {
    DIR *dir;
    struct dirent *ent;
...
void ViewUdiskDataSelect::RootSelectionChanged(GtkTreeSelection *selection) {
    GtkTreeIter iter;
    GtkTreeModel *model;
    gchar *text;
...
void ViewUdiskDataSelect::SetAllToggleValue(GtkTreeModel *model, gboolean value) {
    gboolean valid;
    GtkTreeIter iter;
...
gboolean ViewUdiskDataSelect::CheckBranchStauts(void) {
    return FALSE;
}

gboolean ViewUdiskDataSelect::GetAllSelectPath(void) {
    m_vecPath.clear();
...
#if 0
void ViewUdiskDataSelect::BtnSelectAllClicked(GtkButton *button) {
}

void ViewUdiskDataSelect::BtnDeselectClicked(GtkButton *button) {
}
#endif
